...
CCLE <- PharmacoGx::PharmacoSet(molecularProfiles=z, 
				name="CCLE", 
				cell=celline.ccle, 
				drug=druginfo, 
...
pipeline_input <- data.frame(
  ...
  "uri" = c(
    ...
    transcriptome_link,
    "https://data.broadinstitute.org/ccle_legacy_data/mRNA_expression/CCLE_Expression.Arrays_2013-03-18.tar.gz",
    ...
    "https://github.com/BHKLAB-Pachyderm/get_CCLE/get_CCLEP.R"
  ),
  "access_time" = c(times_rnaseq,times_rnaseq,created, created,created,created,created),
  stringsAsFactors = FALSE
)
...
pipeline_output <- data.frame(
  ...
  "filename" = c("Processed expression data", 
                 ...
                 "Data object"),
  "uri" = c(
    "https://orcestradata.blob.core.windows.net/ccle/CCLE/2015/RNA-seq/",
    "/pfs/processCCLE_Brain-Array/ccle_ge_brainarray_rma.RData",
    "/pfs/downAnnotations/cell_annotation_all.csv",
    "/pfs/downAnnotations/drugs_with_ids.csv",
    "/pfs/ccleRawSensitivity/drug_norm_post.RData",
    "/pfs/calculateccleRAW/slice_$.rds",
    "/pfs/ccleProfilesAssemble/profiles.RData",
    "/pfs/out/CCLE.rds"
  ),
  "access_time" = c(times_rnaseq,created, created,created,created,created,created,created),
  stringsAsFactors = FALSE
)
...
text <- c(
  "Pipeline for creating CCLE data object through ORCESTRA (orcestra.ca), a platform for the reproducible and transparent processing, sharing, and analysis of biomedical data."
)
...
output_subdomain <- data.frame(
  "mediatype" = c("tar.gz","RData","csv", "csv", "RData", "RDS","RData","RDS"),
  "filename" = c("Processed expression data", 
                 "Normalized microarray data",
                 "Downloaded sample annotations",
                 "Downloaded treatment annotations",
...
bco %>% convert_json() %>% export_json("/pfs/out/CCLE_BCO.json") %>% validate_checksum()
